...
def predict(data: Data):
    ...
    temp = np.array(paymentDate)
    try:
        modus = mode(temp[temp != 32].tolist())
        if np.unique(temp).shape[0] == temp.shape[0]:
            modus = "-"
    except: 
        modus = "-"
    ...
    print(paymentDate[paymentDate != 32])
	...
	"Standar Deviasi Tanggal Pembayaran": np.round(float(std_pd), 5),
        ...
        "Modus Tanggal Pembayaran": modus
    }